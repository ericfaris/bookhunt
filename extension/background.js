...
// BookHunt Amazon helper — background service worker.
...
const MENU_ID = 'bookhunt-search';

chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({
    id: MENU_ID,
    title: 'Search this book on BookHunt',
...
function trigger(tabId) {
  ...
  chrome.tabs.sendMessage(tabId, { type: 'bookhunt:open' }).catch(() => {
    // Content script not present (e.g. page loaded before install) — inject then retry.
    chrome.scripting
      .executeScript({ target: { tabId }, files: ['content.js'] })
      .then(() => chrome.tabs.sendMessage(tabId, { type: 'bookhunt:open' }))
      .catch(() => {});
  });
}
